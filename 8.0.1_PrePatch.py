...
# I want to add this line into action.st as a react for trinket proc in BFA
...
# Call the SEF action list if you're using SEF and FoF cd <= 12, Chi >= 3, RSK cd >= 1, OR the target will die within 25 seconds, OR ToD is on the target
...
# Call the SEF action list if you're using SEF and you're not using DHC with FoF cd <= 6, Chi >= 3, RSK cd <= 1, OR the target will die within 15 seconds OR ToD is on the target and you have 1 stack of SEF
...
# Cast ToD cycling through 2 targets if you're using HMFT and your previous GCD was not ToD
...
actions.sef_opener+=/call_action_list,name=cd,if=cooldown.fists_of_fury.remains<=2
# Call actions.cd if:
...
# Cast Fists_of_fury if Rising Sun Kicks remaining cooldown is longer than 1 second and Interrupt Fists_of_fury after 1 tick?
...
# Cast Fist of Fury if the remaining cooldown on rising_sun_kick is longer than the channel duration of Fists_of_fury
